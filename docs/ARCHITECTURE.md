...
|---|---|---|
| 1 | **Persistência de posição do Panel** | Panel é redimensionável e fixável. Se a posição persiste, o que acontece quando o monitor onde ele estava deixa de existir? Hoje persiste só enquanto o processo vive. O Badge, desde que a pastilha virou arrastável, faz o mesmo e ainda menos: a âncora da pilha morre quando a última pastilha sai do ar, porque ela acompanha um estado que acaba e não uma superfície que se reabre. A metade "monitor que sumiu" ele responde — `Monitors.Containing` devolve nulo e a pilha volta ao canto padrão —, mas o dilema do Panel continua aberto. |
...
- [PRODUCT.md](PRODUCT.md) — o que é e por que existe, com as 21 decisões registradas
